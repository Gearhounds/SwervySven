...
public class WheelDrive {
    ...
    public WheelDrive(int angleMotor, int speedMotor, int encoderID) {
        ...
        sPIeD = new PIDController(.0002, 0, 0);
        
        encoder = new CANCoder(encoderID);

        pidController.enableContinuousInput(-180, 180);      
        
        sPIeD.enableContinuousInput(-600000, 600000); 
    }
    ...
    public void SetDirection(double setAngle, double speed) {
        ...
        // double wantedVelocity = MathHelp.map(speed, 0, 1, 0, 16000);

        // speed = sPIeD.calculate(this.speedMotor.getEncoder().getVelocity(), wantedVelocity);
        ...
        double pidOutput = pidController.calculate(currentAngle, setAngle);
        pidOutput *= 0.45;
